...
int main(int, char**)
{
    ...
    for (unsigned int i = 0; i < 3; i++) {
        // 4 = first fairlead id
        auto point = MoorDyn::GetPoints(system, i + 4);
        err = GetPointPos(point, x + 3 * i);
    ...
    err = MoorDynGetNumberLines(system, &n_lines);
    if (err != MOORDYN_SUCCESS) {
        MoorDyn_Close(system);
        return 1;
    }
    for (unsigned int i = 0; i < n_lines; i++) {
        ...
        printf("Line %u\n", line_id);
        printf("=======\n");
        MoorDyn/MoorDynLine line = MoorDyn_GetLine(system, line_id);
        if (!line) {
            MoorDynClose(system);
            return 1;
        }
